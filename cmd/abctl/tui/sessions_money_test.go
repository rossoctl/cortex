...
// THE THREE TITLE-SCOPE TESTS THAT SAT HERE ARE GONE, with the " · lifetime totals" note they
// asserted: that it is present, that it never overflows a narrow terminal, and that it is added
// wherever there is room.
//
// Their premise — that a reader comparing the TOKENS column against the band's window needs the
// table's scope stated — is right, and the WORD was measured to state it wrongly. The session store
// is in memory and resets with the proxy, so on a freshly restarted local proxy the COST column
// summed to $4.04, matching the band's rolling HOUR, while the band's TODAY read $18.80 from the
// durable ledger: a note that sounds like "everything ever" was labelling the shortest span on
// screen, one line under cells that are explicitly clock-windowed. "Lifetime" also names a SPAN,
// and this table has none — each row covers its own session, first event to last.
//
// All three go together rather than one: without the note the overflow guard passes because there
// is nothing to overflow, which is a test that cannot fail.
//
// What states the scope instead is contrast. Every band cell names its own period — LAST 1H, TODAY,
// 7 DAYS, MONTH — so the windowed figures are the labelled ones and the table is the one surface
