...
func TestRefusal_EveryDeclaredRefusalIsClassified(t *testing.T) {
	...
	for name, r := range declared {
		...
		if !classified {
			t.Errorf("%s (%q) is declared but not classified here: decide whether ImpossibleFigure covers it — the halves guard and the disclosure in costing.Settle both read that answer",
				name, r)
			continue
		}
