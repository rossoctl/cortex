...
func TestIncompleteReason(t *testing.T) {
	for _, tc := range []struct {
		...
		// core/costing for the path that proves it never reaches the flag.
...
// nobody's price list.
//
// The SAME four values as core/costing's tierMicros, deliberately: 853fedc3 fixed this
...
// core/costing's TestSettle_PricesEachTierAtItsOwnRate, one layer down — that one goes
