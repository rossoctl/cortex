...
type Counts struct {
	...
	// invariant on event.Event.Avoided forbids adding any of this to spend, to a budget
	...
	//
	// APPLIED savings only. event.Event.TotalAvoidedUSD skips the projected ones and
	...
	// and event.Record's own doc names that as the interesting case; deriving this from
	...
	// gateway reported only a total. event.Event.IncompleteReason carries which, per
...
//	ledger.Fold's       the same residual, per ledger row
//
...
type bucket struct {
	...
	// event.Event.IncompleteReason. Outside the Group machinery for the same
...
// on the session event (see core/cost/event): it prefers the gateway's own
...
// figure, and event.Record is a JSON unmarshal — see Record's call site. The pricedness
// rule is applied here, which is the only thing event.Decode did that Record does not,
// so this arm still means exactly "a priced record was published".
func (a *Aggregator) costOf(e *pipeline.SessionEvent, ce event.Event, haveRec bool) eventCost {
	if haveRec && ce.Priced() {
		...
		// zero are different states here — see event.Event.Tiers.
		...
		if ce.Incomplete {
			...
			// says. Only exactness is withdrawn. See event.Event.Incomplete for why
			...
			// both fields together (settle.Settle sets neither alone), so an empty reason
	...
	if !ok {
		...
		//
		// The header path keeps its causes apart (event.RejectedImplausible) because the
...
//
// THE ORDER MATTERS, for the reason ledger.rowLabel gives: sanitising can triple a
...
//   - ledger.sanitizeLabel is PRIMARY for the durable row — the copy in front of a file
