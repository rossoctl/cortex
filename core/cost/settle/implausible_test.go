	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got.RejectedReason != event.RejectedImplausible {
				t.Errorf("RejectedReason = %q, want %q; without it the refusal reaches no record and nobody can tell a forged figure from an absent one", got.RejectedReason, event.RejectedImplausible)
			}
			// And the record a consumer actually reads.
			rec := NewRecord(got, nil)
			if rec.Priced() {
				t.Error("the published record reads as priced; event.Priced is the guard the ledger writer and the aggregator both use")
			}
			if rec.Micros() != 0 {
				t.Errorf("record Micros() = %d, want 0", rec.Micros())
			}
			if rec.RejectedReason != event.RejectedImplausible {
				t.Errorf("record RejectedReason = %q, want %q", rec.RejectedReason, event.RejectedImplausible)
			}
		})
	}
}
...
func TestSettle_UnparsedEndpointStillSettlesAPlausibleCost(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got.Source != event.SourceGatewayHeader {
				t.Errorf("Source = %q, want %q", got.Source, event.SourceGatewayHeader)
			}
