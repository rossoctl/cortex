...
func TestWriter_UnpricedTrafficIsRecordedWithoutCost(t *testing.T) {
	...
	delete(e.Plugins, event.Key) // no settled cost
	w.Record("s1", e)
...
func TestRecord_AnUnpricedNonInferenceResponseIsStillIgnored(t *testing.T) {
	...
	// admitted deliberately — event.Priced is `CostUSD > 0 || Settled`, because a
	...
	unsettled, merr := json.Marshal(event.Event{Source: event.SourceGatewayHeader})
	if merr != nil {
		t.Fatalf("marshal: %v", merr)
	}
	w.Record("s1", &pipeline.SessionEvent{
		At: at, Phase: pipeline.SessionResponse, StatusCode: 200, Host: "gw.example",
		Plugins: map[string]json.RawMessage{event.Key: unsettled},
	})
	if err := w.Flush(); err != nil {
		t.Fatalf("Flush: %v", err)
	}
