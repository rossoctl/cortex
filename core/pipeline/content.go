...
// MCPExtension, InferenceExtension) to capabilities.ContentSource. The
...
var (
	_ capabilities.ContentSource = (*A2AExtension)(nil)
	_ capabilities.ContentSource = (*MCPExtension)(nil)
	_ capabilities.ContentSource = (*InferenceExtension)(nil)
)

// Fragments implements capabilities.ContentSource for A2A messages.
//
...
func (e *A2AExtension) Fragments() []capabilities.Fragment {
	if e == nil {
		return nil
	}
	var out []capabilities.Fragment
	...
	for _, p := range e.Parts {
		...
		case "text", "data":
			if p.Content != "" {
				out = append(out, capabilities.Fragment{Role: role, Text: p.Content})
			}
		case "file":
	...
	if e.Artifact != "" {
		out = append(out, capabilities.Fragment{Role: capabilities.RoleAssistant, Text: e.Artifact})
	}
	return out
}
...
// the Role constants in core/capabilities (user/assistant). Uniform
...
// classifier compares `f.Role == capabilities.RoleUser` once and works
...
func normalizeA2ARole(r string) string {
	switch r {
	case "agent":
		return capabilities.RoleAssistant
	case "user":
		return capabilities.RoleUser
	default:
...
// Fragments implements capabilities.ContentSource for MCP messages.
//
...
func (e *MCPExtension) Fragments() []capabilities.Fragment {
	if e == nil {
		return nil
	}
	var out []capabilities.Fragment
	...
	if e.Method == "tools/call" && e.Params != nil {
		if name, _ := e.Params["name"].(string); name != "" {
			out = append(out, capabilities.Fragment{Role: capabilities.RoleTool, Text: name})
		}
		if raw, present := e.Params["arguments"]; present {
			...
			if !ok {
				...
				for _, v := range args {
					text := stringifyAny(v)
					if text != "" {
						out = append(out, capabilities.Fragment{Role: capabilities.RoleToolArgs, Text: text})
					}
				}
			}
		}
			...
			if !ok {
				...
				for i, it := range items {
					...
					if t, _ := m["text"].(string); t != "" {
						out = append(out, capabilities.Fragment{Role: capabilities.RoleToolResult, Text: t})
					}
				}
			}
		}
	...
	if e.Err != nil && e.Err.Message != "" {
		out = append(out, capabilities.Fragment{Role: capabilities.RoleToolResult, Text: e.Err.Message})
	}

	return out
}

// Fragments implements capabilities.ContentSource for Inference messages.
//
...
func (e *InferenceExtension) Fragments() []capabilities.Fragment {
	if e == nil {
		return nil
	}
	...
	for _, m := range e.Messages {
		...
		role := m.Role
		if role == "tool" {
			role = capabilities.RoleToolResult
		}
		out = append(out, capabilities.Fragment{Role: role, Text: m.Content})
	}

	if e.Completion != "" {
		out = append(out, capabilities.Fragment{Role: capabilities.RoleAssistant, Text: e.Completion})
	}
	for _, tc := range e.ToolCalls {
		if tc.Name != "" {
			out = append(out, capabilities.Fragment{Role: capabilities.RoleTool, Text: tc.Name})
		}
		if tc.Arguments != "" {
			out = append(out, capabilities.Fragment{Role: capabilities.RoleToolArgs, Text: tc.Arguments})
		}
	}
