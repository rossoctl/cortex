...
11. **Chatty observability traffic and IBAC user intent**: The session store is FIFO with a default cap of 500 events per session, and by default sessions do NOT expire on time (`session.ttl` defaults to never — set it explicitly to bound how long raw prompts stay in memory). A session is dropped only when `session.max_sessions` (100) is exceeded, oldest-used first. Two layered defenses keep the inbound A2A user intent visible to IBAC even when an agent generates dozens of outbound events per turn:
