...
func TestSettle_TruncatedStreamIsDisclosedInexact(t *testing.T) {
	...
	if s.Source != costevent.SourceUsageFallback {
		t.Errorf("Source = %q, want %q", s.Source, costevent.SourceUsageFallback)
	}
...
func TestSettle_GatewayFigureIsNeverIncomplete(t *testing.T) {
	...
	if s.CostUSD != 0.25 || s.Source != costevent.SourceGatewayHeader {
