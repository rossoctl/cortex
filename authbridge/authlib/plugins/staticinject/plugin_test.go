...
func TestConfigure_FailureLeavesZeroDenyState(t *testing.T) {
	p := New()
	// First, a valid configure so we know state would otherwise change.
	if err := p.Configure(json.RawMessage(`{"source":"mappings","mappings":{"api.example.com":"REAL"},"key_by":"host"}`)); err != nil {
		t.Fatalf("Configure() error = %v", err)
	}

	// Now configure with an invalid payload; must fail and must NOT clobber
	// the plugin into some half-built resolver-less state that panics.
	p2 := New()
	if err := p2.Configure(json.RawMessage(`{"source":"bogus"}`)); err == nil {
	...
	action := p2.OnRequest(context.Background(), pctx)
	if action.Type != pipeline.Reject {
		t.Fatalf("OnRequest() on unconfigured plugin action.Type = %v, want %v (Reject, fail-closed)", action.Type, pipeline.Reject)
	}
}
