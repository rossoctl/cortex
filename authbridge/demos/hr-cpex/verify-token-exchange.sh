...
# workday-api audience as the cpex-gateway client, and prints
...
# Expected output:
#   ✓ alice mint                       OK
#   ✓ cpex-gateway → workday-api     OK (aud=workday-api)
...
GATEWAY_CLIENT_ID="cpex-gateway"
GATEWAY_CLIENT_SECRET="cpex-gateway-secret"
...
#    cpex-gateway client. This is exactly the call the
...
if [ -z "$minted" ]; then
  ...
  fail "cpex-gateway → workday-api" "${err}: ${desc}"
  echo
